...
export default function ProjectsSidebar({ projects, activeId, onSwitch, onCreate, onRename, onDelete, onClose }: Props) {
  ...
  const [pendingDeleteId, setPendingDel]  = useState<string | null>(null);
          ...
          <div key={p.id} className="group px-3 mb-1">
            {editingId === p.id ? (
              <div className="flex items-center gap-2 px-3 py-2 rounded-lg"
                ...
                <button onClick={() => submitRename(p.id)} style={{ color: "#27ae60" }}><Check size={13} /></button>
                <button onClick={() => setEditingId(null)} style={{ color: "var(--text-muted)" }}><X size={13} /></button>
              </div>
            ) : (
              <div className="flex items-center gap-3 px-3 py-2.5 rounded-lg cursor-pointer transition-all"
                ...
                <div className="w-2 h-2 rounded-full flex-shrink-0"
                  style={{ background: p.id === activeId ? "var(--gold)" : "var(--text-muted)" }} />
                    ...
                    pendingDeleteId === p.id ? (
                      <div className="flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
                        <button onClick={() => { onDelete(p.id); setPendingDel(null); }}
                          ...
                          style={{ background: "rgba(192,57,43,0.15)", color: "#c0392b" }}>Del</button>
                        <button onClick={() => setPendingDel(null)}
                          className="p-1" style={{ color: "var(--text-muted)" }}><X size={11} /></button>
                      </div>
                    ) : (
      ...
      <div className="px-4 py-4 flex-shrink-0" style={{ borderTop: "1px solid var(--border-subtle)" }}>
        {isCreating ? (
          <div className="flex items-center gap-2 px-3 py-2 rounded-lg"
            ...
            <button onClick={submitCreate} style={{ color: "#27ae60" }}><Check size={14} /></button>
            <button onClick={() => setIsCreating(false)} style={{ color: "var(--text-muted)" }}><X size={14} /></button>
          </div>
        ) : (
